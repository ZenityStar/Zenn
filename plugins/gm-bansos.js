  ...
  try {
    ...
    if (new Date - global.db.data.users[m.sender].lastbansos > 86400000) {
      if (Aku > Kamu) {
        ...
        conn.sendButton( m.chat, `Sorry Gan Lu g Berhasil Korupsi bansos Dan Tidak masuk penjara karna kamu *melarikan diri🏃*`, wm, `𝐊𝐞𝐦𝐛𝐚𝐥𝐢`, `${usedPrefix}menu`, m)
        global.db.data.users[m.sender].lastbansos = new Date * 1
      }
    } else conn.sendButton(m.chat, `Kamu sudah Melakukan Korupsi Bansos, dan kamu harus menunggu selama ${timers} agar bisa korupsi bansos kembali`, wm, `𝐌𝐞𝐧𝐮`, `${usedPrefix}menu`, m)
  } catch (e) {
    throw `${e}`
  }
}
